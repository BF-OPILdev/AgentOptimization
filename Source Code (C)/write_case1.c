...
int main (int argc, char *argv[])
{
    ...
    // write new documents (case 1a, 1b and 1c)
    ...
    printf("Writing case 1b\n");
    // case 1b with flag result_DONE = 1 (will not be processed by optimization)
    doc = bson_new ();
    bson_oid_init (&oid, NULL);
    // BSON_APPEND_OID (doc, "_id", &oid);
    BSON_APPEND_OID (doc, "_id", &oid);
    ...
    printf("Writing case 1c\n");
    // case 1c with flag result_DONE = 0 as case 1a except agent_capacity (agent_capacity = 2)
    // result should be result_optimal_number_of_agents = 1.369, rounded to 2
    doc = bson_new ();
    bson_oid_init (&oid, NULL);
    // BSON_APPEND_OID (doc, "_id", &oid);
    BSON_APPEND_OID (doc, "_id", &oid);
