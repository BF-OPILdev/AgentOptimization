...
int main (int argc, char *argv[])
{
    ...
    // write new documents (case 2a, 2b and 2c)
    ...
    printf("Writing case 2b\n");
    // case 2b with flag result_DONE = 1 (will not be processed by optimization)
    doc = bson_new ();
    bson_oid_init (&oid, NULL);
    BSON_APPEND_OID (doc, "_id", &oid);
    ...
    printf("Writing case 2c\n");
    // case 2c with flag result_DONE = 0 as case 1a except agent_capacity (agent_capacity = 2)
    // result should be result_optimal_number_of_agents = 1.779, rounded to 2
    doc = bson_new ();
    bson_oid_init (&oid, NULL);
    BSON_APPEND_OID (doc, "_id", &oid);
